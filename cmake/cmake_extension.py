...
def is_termux():
    # Termux runs on Android (bionic) but usually reports:
    #   platform.system() == 'Linux'
    #   platform.machine() == 'aarch64'
    # We keep detection conservative and allow users to override defaults
    # via SHERPA_ONNX_CMAKE_ARGS.
...
class BuildExtension(build_ext):
    def build_extension(self, ext: setuptools.extension.Extension):
        ...
        if is_termux():
            # Termux usually doesn't provide ALSA and building portaudio from
            # source may fail due to missing platform-specific deps.
